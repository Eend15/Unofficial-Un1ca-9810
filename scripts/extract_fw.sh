...
{
    local FILE="$1"
    local METADATA="$FW_DIR/${MODEL}_${CSC}/${f}_metadata.txt"
    ...
    echo "partition_size=$(wc -c "$FILE" | cut -d " " -f 1)" >> "$METADATA"

    avbtool info_image --image "$FILE" &> /dev/null || true
    ...
    if [[ "$f" == *"boot.img" ]] || [[ "$f" == "recovery.img" ]]; then
        ...
        if [ $? -ne 0 ]; then
            local FILE_INFO
            LOGW "Failed to parse ${MODEL}_${CSC}/kernel/$f with unpack_bootimg, using legacy bootimg metadata fallback"
            FILE_INFO="$(LC_ALL=C file -b "$FILE")"
            if [[ "$FILE_INFO" == *"cmdline ("* ]]; then
                echo "cmdline=$(sed -n "s/.*cmdline (\(.*\)).*/\1/p" <<< "$FILE_INFO")" >> "$METADATA"
            fi
            if [[ "$FILE_INFO" == *"page size: "* ]]; then
                echo "pagesize=$(sed -n "s/.*page size: \([0-9]*\).*/\1/p" <<< "$FILE_INFO")" >> "$METADATA"
            fi
            if [[ "$FILE_INFO" == *"kernel ("* ]]; then
                echo "kernel_offset=$(sed -n "s/.*kernel (\(0x[0-9a-fA-F]*\)).*/\1/p" <<< "$FILE_INFO")" >> "$METADATA"
            fi
            if [[ "$FILE_INFO" == *"ramdisk ("* ]]; then
                echo "ramdisk_offset=$(sed -n "s/.*ramdisk (\(0x[0-9a-fA-F]*\)).*/\1/p" <<< "$FILE_INFO")" >> "$METADATA"
            fi
            return 0
        fi
        rm -rf "$TMP_DIR/"*
