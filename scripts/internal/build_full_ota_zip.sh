...
GENERATE_UPDATER_SCRIPT()
{
...

    {
        ...
        if [ -f "$SRC_DIR/target/$TARGET_CODENAME/installer/install-start.edify" ]; then
            cat "$SRC_DIR/target/$TARGET_CODENAME/installer/install-start.edify"
        fi
...
TARGET_UPDATER_BINARY="$SRC_DIR/target/$TARGET_CODENAME/installer/update-binary"
if [ -f "$TARGET_UPDATER_BINARY" ]; then
    LOG "- Using target updater-binary"
    EVAL "cp -a \"$TARGET_UPDATER_BINARY\" \"$TMP_DIR/META-INF/com/google/android/update-binary\"" || exit 1
fi
