...
BUILD_IMAGE_MKFS()
{
    ...
    case "$FS_TYPE" in
        "ext4")
            ...
            # e2fsdroid looks up the input directory root as an empty path in
            # canned fs_config. Some legacy target partitions only carry the
            # mount-point entry (e.g. odm), so mirror that entry for the root.
            ...
            # for e2fsdroid lookups that would otherwise fail.
