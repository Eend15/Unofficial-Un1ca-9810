...
def build_boot(image, ramdisk, compress=True, keep_extra_tail=False):
    ...
    if len(result) > len(image):
        raise ValueError("logger BOOT exceeds the supplied BOOT image size")
    return bytes(result)
...
def main():
    ...
    if args.format == "auto":
        compress = not (is_cpio(ramdisk) and not is_gzip(ramdisk))
    ...
    patched_ramdisk = replace_init(ramdisk, logger, args.drop_entry)
    if not args.no_debug_ramdisk:
