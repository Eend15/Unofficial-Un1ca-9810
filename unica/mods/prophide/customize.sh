...
if [ ! -f "$VBIMG" ]; then
    LOGW "Skipping Hide sensitive props because target firmware does not ship AVB vbmeta.img"
    DELETE_FROM_WORK_DIR "system" "system/bin/prophide.sh"
    DELETE_FROM_WORK_DIR "system" "system/bin/rezetprop"
    DELETE_FROM_WORK_DIR "system" "system/etc/init/prophide.rc"
    unset VBIMG
