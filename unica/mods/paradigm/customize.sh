...
# Game Booster
# Optional, fetched live from Samsung's Galaxy Store CDN. That endpoint returns
# a URL for only one of the probed device models and blips intermittently, so a
# hard failure here would abort an otherwise-good 8-minute build over a
# non-essential app. Treat a failed fetch as a warning and carry on.
LOG "- Downloading latest Game Booster app"
_GAMETOOLS_URL="$(GET_GALAXY_STORE_DOWNLOAD_URL "com.samsung.android.game.gametools" 2> /dev/null)"
if [ -n "$_GAMETOOLS_URL" ] && \
        DOWNLOAD_FILE "$_GAMETOOLS_URL" \
            "$WORK_DIR/system/system/priv-app/GameTools_Dream/GameTools_Dream.apk"; then
    :
else
    LOGW "Game Booster download unavailable; continuing without it"
fi
unset _GAMETOOLS_URL
