...
if ! MKBOOTIMG_ARGS="$(unpack_bootimg --boot_img "$TMP_DIR/boot.img" --out "$TMP_DIR/out" --format mkbootimg 2>&1)" || \
        [ ! -f "$TMP_DIR/out/kernel" ]; then
    if [ "$TARGET_PLATFORM" = "exynos9810" ]; then
        LOGW "Skipping PROCA kernel patch because legacy exynos9810 boot.img could not be parsed"
        EVAL "rm -rf \"$TMP_DIR\""
...
if [[ "$(LC_ALL=C file -b "$TMP_DIR/out/kernel")" != "Linux kernel ARM64"* ]]; then
    if [ "$TARGET_PLATFORM" = "exynos9810" ]; then
        LOGW "Skipping PROCA kernel patch because extracted exynos9810 kernel image is not a raw ARM64 image"
        EVAL "rm -rf \"$TMP_DIR\""
        unset MKBOOTIMG_ARGS GZ_COMPRESSED
        return 0
    fi
