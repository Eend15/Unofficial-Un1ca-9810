...
EXYNOS9810_EMBEDDED_PORT_DIR="$EXYNOS9810_PATCH_DIR/embedded/exynos9810_legacy_port"
...
if [ "$EXYNOS9810_ALLOW_EXTERNAL_DONOR" = "true" ] && \
        [ -d "$EXYNOS9810_LEGACY_PORT_DIR/device_port" ] && \
        [ -d "$EXYNOS9810_LEGACY_PORT_DIR/vendor" ]; then
    LOG "Using explicitly selected Exynos9810 donor overlay"
else
    # The normal build must be reproducible on a clean PC. Never fall back to a
    # Downloads path or silently skip the device stack: use the payload stored
    # beside this module instead.
    EXYNOS9810_LEGACY_PORT_DIR="$EXYNOS9810_EMBEDDED_PORT_DIR"
    if [ ! -d "$EXYNOS9810_LEGACY_PORT_DIR/device_port" ] || \
            [ ! -d "$EXYNOS9810_LEGACY_PORT_DIR/vendor" ]; then
        LOGE "Embedded Exynos9810 boot baseline is missing: $EXYNOS9810_LEGACY_PORT_DIR"
        return 1
    fi
...
EOF
    ...
    # Bixby button = scancode 703 on starlte, star2lte and crownlte. USER is a
    # neutral Samsung keycode carrier: PhoneWindowManager intercepts the
...
_EXYNOS9810_PATCH_DEBLOATED_FLOATING_FEATURES()
{
    ...
    for FILE in \
        ...
        # This is the tested panel capability for the Exynos9810 stack. Apply
        # it after the legacy floating-feature restore
...
# The 9810 devices have no eUICC. Clear this persistent donor property before
# the Samsung RIL starts so a dirty flash cannot retain TSDS/eSIM slot mode.
on post-fs-data
...
_EXYNOS9810_APPLY_BOOT_PROPS()
{
    ...
    # Keep the public author neutral; the Exynos9810 port does not expose a
    # private donor identity through the system properties.
    _EXYNOS9810_SET_PROP_ALL "ro.product.author" "UN1CA"
    _EXYNOS9810_SET_PROP_ALL "ro.unica9810.name" "UN1CA Exynos9810"
...
_EXYNOS9810_WRITE_SYSTEM_BOOT_DEBUG()
{
    ...
    LOG "- Adding safe Exynos9810 second-stage boot logger"
...
on early-init
    write /dev/kmsg "UN1CA-9810-BOOT: early-init"

on init
    write /dev/kmsg "UN1CA-9810-BOOT: init"

on fs
    write /dev/kmsg "UN1CA-9810-BOOT: fs"

on post-fs
    write /dev/kmsg "UN1CA-9810-BOOT: post-fs"
    ...
    copy /proc/self/mountinfo /cache/unica/mountinfo_post_fs.txt
    copy /proc/partitions /cache/unica/partitions.txt

on late-fs
    write /dev/kmsg "UN1CA-9810-BOOT: late-fs"

on post-fs-data
    write /dev/kmsg "UN1CA-9810-BOOT: post-fs-data"
    mkdir /cache/unica 0777 root root
...
on zygote-start
    write /dev/kmsg "UN1CA-9810-BOOT: zygote-start"

on boot
    write /dev/kmsg "UN1CA-9810-BOOT: boot"

on property:sys.boot_completed=1
    write /dev/kmsg "UN1CA-9810-BOOT: sys.boot_completed=1"
    mkdir /cache/unica 0777 root root
...
# This service is deliberately ordinary second-stage init. It never mounts a
# partition, edits BOOT, changes properties, or touches private key material.
# It only reads the state that init and fs_mgr have already established.
RUN_ID="$(date '+%Y%m%d-%H%M%S' 2>/dev/null || echo boot)-$$"
DATA_OUT=/data/bootloop-logs/late_boot_$RUN_ID

# post-fs-data normally has /data mounted. Prefer it immediately so a reboot
# cannot erase the useful late-boot tail while a growing cache tree is copied.
if mkdir -p "$DATA_OUT" 2>/dev/null && [ -w "$DATA_OUT" ]; then
    OUT="$DATA_OUT"
else
    OUT=/cache/unica/late_boot_$RUN_ID
    if ! mkdir -p "$OUT" 2>/dev/null || [ ! -w "$OUT" ]; then
        OUT=/metadata/unica/late_boot_$RUN_ID
...
collect_keymaster()
{
    ...
    # Do not dump private key blobs. Their existence, ownership, labels and
    # hashes are sufficient for diagnosing missing/incorrect keystore state.
    {
        echo
        echo "private security-state metadata (contents intentionally omitted):"
...
{
    TAG="$1"
    DEST="$OUT/runtime_artifacts_${TAG}.txt"
    : > "$DEST"
    for BASE in /data/anr /data/tombstones /data/system/dropbox /data/misc/logd /sys/fs/pstore; do
        [ -d "$BASE" ] || continue
        find "$BASE" -maxdepth 2 -type f -print 2>/dev/null | while IFS= read -r PATH_NAME; do
            {
                echo
                echo "===== runtime artifact: $PATH_NAME ====="
                ls -lZ "$PATH_NAME" 2>&1 || true
                sha256sum "$PATH_NAME" 2>&1 || true
                # Keep the diagnostic archive bounded while retaining the end
                # of ANR/tombstone/dropbox files where the fatal exception is.
                tail -c 1048576 "$PATH_NAME" 2>&1 || true
            } >> "$DEST"
        done
    done
}
...
state_monitor()
{
    while :; do
        {
            echo
            echo "===== late Android state $(date '+%Y-%m-%d %H:%M:%S' 2>/dev/null) ====="
            echo "uptime=$(cat /proc/uptime 2>/dev/null || true)"
            getprop | grep -E '^\[(sys\.boot_completed|sys\.boot_reason|sys\.powerctl|ro\.boot\.bootreason|ro\.boot\.rebootcause|init\.svc\.system_server|init\.svc\.zygote|init\.svc\.surfaceflinger|init\.svc\.audioserver|init\.svc\.vendor\.radio-hal)' || true
            echo "system_server_pid=$(pidof system_server 2>/dev/null || true)"
            echo "zygote_pid=$(pidof zygote 2>/dev/null || true)"
            echo "surfaceflinger_pid=$(pidof surfaceflinger 2>/dev/null || true)"
            echo "bootanimation_pid=$(pidof bootanimation 2>/dev/null || true)"
            echo "top processes:"
            ps -A -o PID,PPID,USER,STATE,NAME 2>/dev/null | grep -E 'system_server|zygote|surfaceflinger|bootanimation|watchdog|lmkd|audioserver|cameraserver|netd|vold' || true
            echo "recent kernel warnings:"
            dmesg 2>/dev/null | tail -160 | grep -iE 'panic|fatal|watchdog|deadlock|oom|out of memory|reboot|shutdown|powerctl|erofs|ext4|f2fs|dm-verity|avb|keymaster|gatekeeper|selinux|denied' || true
        } >> "$OUT/late_state.log" 2>&1
        copy_to_data
        sync
        sleep 5
    done
...
snapshot()
{
    ...
    collect_avb_and_storage "$TAG"
    collect_init_and_services "$TAG"
    {
        echo "UN1CA Exynos9810 detailed boot snapshot: $TAG"
        date
        echo
        echo "uptime:"
        cat /proc/uptime
        ...
        echo
        echo "psi/cgroup state:"
        find /sys/fs/cgroup -maxdepth 2 -type f \( -name 'memory.*' -o -name 'cpu.*' -o -name 'io.*' \) -print 2>/dev/null | sort -u | while IFS= read -r PATH_NAME; do
            echo "-- $PATH_NAME --"
            cat "$PATH_NAME" 2>/dev/null || true
        done
        echo
        echo "properties:"
        getprop
        ...
        dmesg
        echo
        echo "boot logger files:"
        find /cache /metadata /data -maxdepth 3 -type f \( -iname '*boot*' -o -iname '*pstore*' \) -print 2>/dev/null || true
    } > "$OUT/boot_${TAG}.txt" 2>&1

    logcat -b all -d -v threadtime > "$OUT/logcat_${TAG}.txt" 2>&1
    ...
    run_limited dumpsys activity services > "$OUT/dumpsys_activity_services_${TAG}.txt" 2>&1 || true
    run_limited dumpsys activity processes > "$OUT/dumpsys_activity_processes_${TAG}.txt" 2>&1 || true
    run_limited dumpsys window > "$OUT/dumpsys_window_${TAG}.txt" 2>&1 || true
    run_limited dumpsys SurfaceFlinger > "$OUT/dumpsys_surfaceflinger_${TAG}.txt" 2>&1 || true
    run_limited dumpsys media.audio_flinger > "$OUT/dumpsys_audio_${TAG}.txt" 2>&1 || true
    run_limited dumpsys telephony.registry > "$OUT/dumpsys_telephony_${TAG}.txt" 2>&1 || true
    run_limited dumpsys dropbox --print > "$OUT/dumpsys_dropbox_${TAG}.txt" 2>&1 || true
    collect_runtime_artifacts "$TAG"
    copy_to_data
    event "snapshot $TAG finished"
}

event "logger started; output=$OUT"
snapshot post-fs
...
kill "$STATE_MONITOR_PID" 2>/dev/null || true
wait "$STATE_MONITOR_PID" 2>/dev/null || true
snapshot final
event "logger finished; sys.boot_completed=$(getprop sys.boot_completed)"
copy_to_data
sync
EOF
...
on property:sys.boot_completed=1
    write /dev/kmsg "UN1CA-9810-BOOT: boot_completed"
    write /cache/unica/boot_completed.txt "1"
...
_EXYNOS9810_RESTORE_BOOT_VENDOR_SHIMS()
{
    ...
    for REL in \
        "lib/libexynos9810compat.so" \
        "lib64/libexynos9810compat.so" \
        "lib64/vendor.samsung.hardware.security.vaultkeeper@2.0.so"; do
        _EXYNOS9810_COPY_FILE "$EXYNOS9810_LEGACY_PORT_DIR/vendor/$REL" \
            "$WORK_DIR/vendor/$REL" \
            "vendor" "vendor/$REL" "/vendor/$REL" "u:object_r:vendor_file:s0"
...
_EXYNOS9810_RESTORE_BOOT_SYSTEM_SHIMS()
{
    ...
    _EXYNOS9810_COPY_FILE "$EXYNOS9810_LEGACY_PORT_DIR/device_port/device/common/system/lib/libexynos9810compat.so" \
        "$WORK_DIR/system/system/lib/libexynos9810compat.so" \
        "system" "system/lib/libexynos9810compat.so" "/system/lib/libexynos9810compat.so" "u:object_r:system_lib_file:s0"
    [ -f "$WORK_DIR/system/system/lib/libexynos9810compat.so" ] && \
        _EXYNOS9810_SET_METADATA "system" "system/lib/libexynos9810compat.so" "/system/lib/libexynos9810compat.so" \
            0 0 644 "u:object_r:system_lib_file:s0"

    _EXYNOS9810_COPY_FILE "$EXYNOS9810_LEGACY_PORT_DIR/device_port/device/common/system/lib64/libexynos9810compat.so" \
        "$WORK_DIR/system/system/lib64/libexynos9810compat.so" \
        "system" "system/lib64/libexynos9810compat.so" "/system/lib64/libexynos9810compat.so" "u:object_r:system_lib_file:s0"
    [ -f "$WORK_DIR/system/system/lib64/libexynos9810compat.so" ] && \
        _EXYNOS9810_SET_METADATA "system" "system/lib64/libexynos9810compat.so" "/system/lib64/libexynos9810compat.so" \
            0 0 644 "u:object_r:system_lib_file:s0"
}
...
on property:sys.boot_completed=1
    write /dev/kmsg "UN1CA-9810-TRACE: sys.boot_completed=1"
EOF
...
{
    # A previous build silently produced a much smaller vendor image because
    # the donor tree was incomplete. Fail at the overlay stage, while the
    # missing path is still actionable, instead of producing a late bootloop.
    [ "$EXYNOS9810_HXA3_VENDOR_BASE_APPLIED" = "true" ] && return 0

    local SRC_COUNT DST_COUNT REL
    SRC_COUNT="$(find "$EXYNOS9810_LEGACY_PORT_DIR/vendor" -type f -print 2>/dev/null | wc -l)"
    DST_COUNT="$(find "$WORK_DIR/vendor" -type f -print 2>/dev/null | wc -l)"
    if [ "$DST_COUNT" -lt "$SRC_COUNT" ]; then
        LOGE "Embedded Exynos9810 vendor baseline was truncated: source=$SRC_COUNT work=$DST_COUNT"
        return 1
    fi
    ...
    for REL in \
        ...
        etc/fstab.exynos9810 \
        etc/fstab.samsungexynos9810 \
        etc/init/init.samsungexynos9810.rc \
        etc/selinux/vendor_sepolicy.cil \
        lib64/libkeymaster4.so \
        lib64/libMcClient.so; do
        [ -f "$WORK_DIR/vendor/$REL" ] || {
            LOGE "Known-booting Exynos9810 vendor component is missing: vendor/$REL"
            return 1
        }
    done

    # Preserve the execution domains used by the reference init services.
    # Generic vendor_file labels are sufficient for data files, but an
    # enforcing build cannot start these HALs when their exec labels are lost.
    _EXYNOS9810_SET_METADATA "vendor" \
        "vendor/bin/hw/android.hardware.keymaster@4.0-service" \
        "/vendor/bin/hw/android.hardware.keymaster@4.0-service" \
        0 2000 755 "u:object_r:hal_keymaster_default_exec:s0"
    ...
    _EXYNOS9810_SET_METADATA "vendor" "vendor/bin/hw/gpsd" \
        "/vendor/bin/hw/gpsd" 0 2000 755 "u:object_r:gpsd_exec:s0"
    _EXYNOS9810_SET_METADATA "vendor" "vendor/bin/hw/lhd" \
        "/vendor/bin/hw/lhd" 0 2000 755 "u:object_r:lhd_exec:s0"
    _EXYNOS9810_SET_METADATA "vendor" "vendor/bin/mcDriverDaemon" \
        "/vendor/bin/mcDriverDaemon" 0 0 755 "u:object_r:mobicore_exec:s0"
    for REL in \
        ...
        vendor/etc/selinux/vendor_sepolicy_version; do
        _EXYNOS9810_SET_METADATA "vendor" "$REL" "/$REL" \
            0 0 644 "u:object_r:vendor_configs_file:s0"
    done
}
